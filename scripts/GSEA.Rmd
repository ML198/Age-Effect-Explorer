...
doGSEA <- function(pval_table, gene_list, tissue_name = "",
  ...
  .prep_and_run <- function(df, which_dir = c("up", "down")) {
    ...
    if (n < 100) {
      ...
      sub <- dplyr::slice_head(sub, n = 1000)
    }
  ...
  if (is.null(gsea_res) || nrow(gsea_res) == 0) {
    return(list(result = tibble::tibble(), plot = ggplot() +
      theme_void()))
  }
  ...
  gsea_res <- gsea_res %>%
    ...
    # build a clean label used only for plotting
    dplyr::mutate(
      pathway_label = stringr::str_wrap(gsub("_", " ", .data$pathway), width = 40)
    )
...
make_and_save_tables <- function(gsea_res, tissue_name,
  ...
  if (is.null(gsea_res) || nrow(gsea_res) == 0) {
    keep_df <- tibble::tibble(
      ...
      NES = numeric(), pval = numeric(), padj = numeric()
    )
  } else {
    # safety: attach ID if missing
    if (!"pathway_id" %in% names(gsea_res)) {
      gsea_res <- dplyr::left_join(gsea_res, pathway_map,
        by = c("pathway" = "pathway_desc")
      )
    }
    keep_df <- gsea_res %>%
  ...
  outfile <- file.path(out_dir, sprintf(
    "GSEA_%s_padj%s.csv",
    tissue_name, padj_thresh
  ))
...
gsea_plots <- lapply(tissues, function(t) {
  ...
  pval_table <- read.csv(file.path(result_dir, sprintf("%s_pvalue_results.csv", tissue_name)), header = TRUE, na.strings = c("", "NA", "--", "Inf", "-Inf"))
  res <- doGSEA(
