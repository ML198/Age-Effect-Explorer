...
tissues <- (c("muscle skeletal", "whole blood"))
  ...
  pval_df <- exp_long %>%
    ...
    group_modify(~ {
      ...
      if (nrow(df_sub) == 0 || sd(df_sub$log2TPM, na.rm = TRUE) < 1e-6 || mean(df_sub$log2TPM == 0, na.rm = TRUE) > 0.1) {
        return(tibble(p_value_age = NA_real_, p_value_sex = NA_real_, p_value_age_sex = NA_real_, age_coef = NA_real_, sex_coef = NA_real_, `sex effect` = NA_character_, `age effect` = NA_character_))
      }
      ...
      fit <- tryCatch({
        lm(as.formula(paste("log2TPM ~ sex * age +", paste(model_vars, collapse = " + "))), data = na.omit(df_sub))
      ...
      p_value_age_sex <- tidy_res %>% filter(term == "sexMale:age") %>% pull(p.value)
      ...
      tibble(
        ...
        p_value_age_sex = if (length(p_value_age_sex) == 0) NA_real_ else p_value_age_sex,
    ...
    mutate(
      ...
      BH_adjusted_age_sex = p.adjust(p_value_age_sex, method = 'BH')
    ) %>%
   # filter(BH_adjusted_age < 0.05 & !is.na(BH_adjusted_age) & BH_adjusted_age != "NA") %>% 
    filter(!is.na(BH_adjusted_age) & BH_adjusted_age != "NA") %>% 
    mutate(
      ...
      p_value_age_sex = formatC(signif(p_value_age_sex,2), format = "e", digits = 2),
      BH_adjusted_age_sex = formatC(signif(BH_adjusted_age_sex, 2), format = "e", digits = 2),
    ) %>%
    select(Rank, Gene, intercept, BH_adjusted_age, BH_adjusted_sex, BH_adjusted_age_sex, p_value_age, age_coef, `age effect`, p_value_sex, sex_coef, `sex effect`, p_value_age_sex)
